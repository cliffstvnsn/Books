...
class Book:
    ...
    def get_by_id(cls,data):
        ...
        for row in results:
            ...
            book.favorite_books.append(author_model.Author(data))
        return book

    @classmethod
    def unfavorited_books(cls,data):
        query = "SELECT * FROM books WHERE books.id NOT IN ( SELECT book_id FROM favorite_books WHERE user_id = %(id)s );"
        results = connectToMySQL('books').query_db(query,data)
        books = []
        for row in results:
            books.append(cls(row))
